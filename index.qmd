...
analysis. The Survival package was used to consturct the Cox regression
...
group with covariate values x~1~ and x~2~ are given by by hx~1~(t) =
h~0~(t) exp(βx~1~) and hx~2~(t) = h~0~(t) exp(βx~2~) respectively

The hazard ratio is expressed as: HR = hx~2~(t)/ hx~1~(t) = exp\[β(x~2~-
x~1~)\]
